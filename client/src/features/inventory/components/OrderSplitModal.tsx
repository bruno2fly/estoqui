...
/* ────────────────────────────────────────────────────────────
   VendorListModal — editable popup when clicking "See list"
   ──────────────────────────────────────────────────────────── */
function VendorListModal({
  ...
  // Sync local state when group changes or modal opens
  useEffect(() => {
    if (group) {
      setEditLines(group.lines.map((l) => ({ ...l })))
      setHasChanges(false)
    }
  }, [group, open])
  ...
  const handleQtyChange = useCallback((index: number, value: string) => {
    ...
    setEditLines((prev) =>
      prev.map((l, i) =>
        i === index ? { ...l, qty, lineTotal: qty * l.unitPrice } : l
      )
    )
    setHasChanges(true)
  }, [])
  ...
  const handleSave = useCallback(() => {
    if (!group) return
    const validLines = editLines.filter((l) => l.qty > 0)
    onSave(group.vendorId, validLines)
    setHasChanges(false)
    onClose()
  }, [group, editLines, onSave, onClose])

  if (!group) return null

  const subtotal = editLines.reduce((s, l) => s + l.lineTotal, 0)

  return (
    <Modal open={open} onClose={onClose} title={group.vendor.name} maxWidth="750px">
              ...
              editLines.map((l, i) => {
                ...
                return (
                  <tr key={`${l.productId}-${i}`} className="border-b border-surface-border/50 hover:bg-surface-hover/50">
                    ...
                    <td className="px-3 py-1.5 text-right text-muted tabular-nums">$ {l.unitPrice.toFixed(2)}</td>
                    <td className="px-3 py-1.5 text-right font-medium text-fg tabular-nums">$ {l.lineTotal.toFixed(2)}</td>
      ...
      <div className="pt-3 mt-3 border-t border-surface-border flex items-center justify-between">
        ...
        <span className="text-sm font-semibold text-fg tabular-nums">Subtotal: $ {subtotal.toFixed(2)}</span>
      </div>
    </Modal>
  )
}
...
export function OrderVendorCards({
  ...
  const updateOrderVendorLines = useStore((s) => s.updateOrderVendorLines)
  ...
  return (
    <>
      <div className="bg-surface border border-surface-border rounded-2xl p-6 shadow-sm space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="flex size-10 items-center justify-center rounded-xl bg-success-bg text-success">
            ...
            <div>
              <h2 className="text-base font-semibold text-fg">Order Created</h2>
              <p className="text-xs text-fg-secondary">Send each vendor their list via WhatsApp, email, or CSV</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {onReset && (
              <button
                type="button"
                onClick={onReset}
                className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-[12px] font-medium text-danger border border-danger/30 hover:bg-danger-bg transition-colors"
              >
                <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <button
              type="button"
              onClick={onArchive}
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[12px] font-medium bg-primary text-primary-foreground shadow-sm hover:bg-primary-hover transition-colors"
            >
              <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {/* Summary bar */}
        <div className="flex items-center gap-5 bg-accent border border-primary/20 rounded-xl px-4 py-3">
          <div className="flex items-center gap-1.5">
            <span className="text-xl font-bold text-primary tabular-nums">{totalVendors}</span>
            <span className="text-xs text-muted">vendors</span>
          </div>
          <div className="w-px h-6 bg-surface-border" />
          <div className="flex items-center gap-1.5">
            <span className="text-xl font-bold text-fg tabular-nums">{totalItems}</span>
            <span className="text-xs text-muted">items</span>
          </div>
          <div className="w-px h-6 bg-surface-border" />
          <div className="flex items-center gap-1.5">
            <span className="text-xl font-bold text-success tabular-nums">$ {order.total.toFixed(2)}</span>
          ...
          <div className="ml-auto">
            <button
              ...
              className="text-xs font-medium text-primary hover:text-primary-hover transition-colors px-2.5 py-1.5 rounded-lg border border-primary/30 hover:bg-accent"
            >
              Copy All WhatsApp
            </button>
          </div>
              ...
              >
                ...
                <div className="flex gap-1.5">
                  <button
                    ...
                    className="inline-flex items-center gap-1 px-2 py-1.5 rounded-lg text-[11px] font-medium bg-success-bg text-success border border-success/30 hover:bg-success/15 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 px-2 py-1.5 rounded-lg text-[11px] font-medium bg-accent text-accent-foreground border border-primary/30 hover:bg-accent/70 transition-colors"
                  >
                    <svg className="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* Vendor detail popup — editable */}
      <VendorListModal
        open={viewingVendor !== null}
        onClose={() => setViewingVendor(null)}
        group={viewingGroup}
        onSave={(vendorId, lines) => {
          updateOrderVendorLines(vendorId, lines)
          toast.show(lines.length > 0 ? 'Order updated!' : 'Vendor removed from order')
        }}
      />
    </>
  )
