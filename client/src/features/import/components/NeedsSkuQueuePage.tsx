    ...
    <div className="space-y-6">
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <input
          ...
          className="bg-input-bg border border-input-border text-fg px-2.5 py-2 rounded-lg text-[12px] w-40 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary"
        />
        <input
          type="number"
          value={confidenceMin}
          ...
          min={0}
          max={100}
          className="bg-input-bg border border-input-border text-fg px-2.5 py-2 rounded-lg text-[12px] w-28 tabular-nums focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary"
        />
      </div>

      {selectedIds.size > 0 && (
        <div className="flex items-center gap-3 bg-accent border border-primary/20 rounded-xl p-3">
          <span className="text-[13px] font-medium text-fg">{selectedIds.size} selected</span>
          <button
            type="button"
            onClick={() => setBulkSkuModal(true)}
            className="px-3.5 py-1.5 rounded-xl bg-primary text-primary-foreground text-[12px] font-medium shadow-sm hover:bg-primary-hover transition-colors"
          >
            Assign same SKU
          </button>
          <button
            ...
            className="px-3.5 py-1.5 rounded-xl text-danger text-[12px] font-medium hover:bg-danger-bg transition-colors"
          >
            Ignore selected
          </button>
          <button
            ...
            className="ml-auto px-3.5 py-1.5 rounded-xl text-fg-secondary text-[12px] hover:bg-surface-hover transition-colors"
          >
            Clear
          </button>
        </div>
      ...
      <div className="bg-surface border border-surface-border rounded-2xl p-6 shadow-sm flex gap-5">
        <div className="flex-1 min-w-0">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr>
                  <th className="text-left px-3 py-3 w-8 bg-surface-hover/40 border-b border-surface-border">
                    <input
                      type="checkbox"
                  ...
                  <th className="text-left text-muted font-semibold text-[11px] uppercase tracking-wider px-3 py-3 bg-surface-hover/40 border-b border-surface-border">Product</th>
                  <th className="text-left text-muted font-semibold text-[11px] uppercase tracking-wider px-3 py-3 bg-surface-hover/40 border-b border-surface-border">Brand</th>
                  <th className="text-left text-muted font-semibold text-[11px] uppercase tracking-wider px-3 py-3 bg-surface-hover/40 border-b border-surface-border">Barcode</th>
                  <th className="text-left text-muted font-semibold text-[11px] uppercase tracking-wider px-3 py-3 bg-surface-hover/40 border-b border-surface-border">Price</th>
                  <th className="text-left text-muted font-semibold text-[11px] uppercase tracking-wider px-3 py-3 bg-surface-hover/40 border-b border-surface-border">Conf.</th>
                  <th className="text-right text-muted font-semibold text-[11px] uppercase tracking-wider px-3 py-3 bg-surface-hover/40 border-b border-surface-border">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((r) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 text-[13px] text-fg tabular-nums">{r.barcode ?? '—'}</td>
                    <td className="px-3 py-2.5 text-[13px] text-fg tabular-nums">$ {r.price.toFixed(2)}</td>
                    <td className="px-3 py-2.5 text-[13px] text-fg tabular-nums">{r.confidence}%</td>
                    <td className="px-3 py-2.5 text-right" onClick={(e) => e.stopPropagation()}>
                      <button
                        type="button"
                        onClick={() => setAssignRow(r)}
                        className="px-2.5 py-1 rounded-lg bg-primary text-primary-foreground text-[11px] font-medium shadow-sm hover:bg-primary-hover transition-colors"
                      >
                        Assign SKU
                      </button>
                    </td>
        ...
        {drawerRow && (
          <div className="w-80 shrink-0 border-l border-surface-border pl-5 flex flex-col">
            ...
            <div className="space-y-3 text-[13px] text-fg">
              ...
              <p className="tabular-nums"><span className="text-muted">Barcode:</span> {drawerRow.barcode ?? '—'}</p>
              <p className="tabular-nums"><span className="text-muted">Price:</span> $ {drawerRow.price.toFixed(2)}</p>
              <p className="tabular-nums"><span className="text-muted">Confidence:</span> {drawerRow.confidence}%</p>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {drawerRow.proposedMatches.map((m) => (
                      <span
                        key={m.sku}
                        className="px-2 py-0.5 rounded-md bg-surface-hover text-[11px] text-fg-secondary"
                      >
                        {m.sku} ({m.score})
                      </span>
                    ))}
            ...
            <div className="mt-4 space-y-2">
              ...
              <button
                ...
                className="w-full py-2 rounded-xl bg-primary text-primary-foreground text-[12px] font-medium shadow-sm hover:bg-primary-hover transition-colors"
              >
                Assign SKU
              </button>
            </div>
      ...
      {bulkSkuModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 dark:bg-black/70 p-5 backdrop-blur-sm">
          <div className="bg-surface border border-surface-border rounded-2xl w-full max-w-sm p-6 shadow-2xl">
            ...
            <input
              ...
              className="w-full bg-input-bg border border-input-border text-fg px-3 py-2 rounded-lg text-[13px] mb-4 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary"
            />
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => { setBulkSkuModal(false); setBulkSku('') }}
                className="flex-1 py-2 rounded-xl border border-surface-border text-fg-secondary hover:bg-surface-hover transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 py-2 rounded-xl bg-primary text-primary-foreground font-medium shadow-sm hover:bg-primary-hover transition-colors disabled:opacity-50"
              >
                Assign
              </button>
            </div>
